...
class Diglin_Csstidy_Optimise
{
    /**
     * Constructor
     * @param Diglin_Csstidy_Core $css contains the class csstidy
     * @access private
     * @version 1.0
     */
    ...
    public function subvalue()
    {
        ...
        $important = '';
        if(Diglin_Csstidy_Core::is_important($this->sub_value))
        {
            $important = '!important';
        }
        $this->sub_value = Diglin_Csstidy_Core::gvw_important($this->sub_value);
    ...
    {
        $important = '';
        if(Diglin_Csstidy_Core::is_important($value))
        {
            $values = Diglin_Csstidy_Core::gvw_important($value);
            $important = '!important';
        }
        else $values = $value;
    ...
    {
        if(Diglin_Csstidy_Core::is_important($string))
        {
            $string = Diglin_Csstidy_Core::gvw_important($string) . '!important';
        }
        return $string;
    }

    /**
     * Color compression public function. Converts all rgb() values to #-values and uses the short-form if possible. Also replaces 4 color names by #-values.
     * @param string $color
     * @return string
     * @version 1.1
     */
    ...
    public function dissolve_4value_shorthands($property,$value)
    {
        ...
        if(Diglin_Csstidy_Core::is_important($value))
        {
            $value = Diglin_Csstidy_Core::gvw_important($value);
            $important = '!important';
        }
    ...
    public function explode_ws($sep,$string)
    {
        ...
        for($i = 0, $len = strlen($string);$i < $len; $i++)
        {
            ...
            {
                case 'st':
                if($string{$i} == $sep && !Diglin_Csstidy_Core::escaped($string,$i))
                {
                    ++$num;
                }
                elseif($string{$i} == '"' || $string{$i} == '\'' || $string{$i} == '(' && !Diglin_Csstidy_Core::escaped($string,$i))
                {
                    $status = 'str';
                ...
                case 'str':
                if($string{$i} == $to && !Diglin_Csstidy_Core::escaped($string,$i))
                {
                    $status = 'st';
                }
                (isset($output[$num])) ? $output[$num] .= $string{$i} : $output[$num] = $string{$i};
    ...
    public function merge_4value_shorthands($array)
    {
        ...
        foreach($shorthands as $key => $value)
        {
            ...
            && isset($array[$value[2]]) && isset($array[$value[3]]) && $value !== 0)
            {
                ...
                {
                    $val = $array[$value[$i]];
                    if(Diglin_Csstidy_Core::is_important($val))
                    {
                        $important = '!important';
                        $return[$key] .= Diglin_Csstidy_Core::gvw_important($val).' ';
                    }
                    else
                    {
                        $return[$key] .= $val.' ';
    ...
    public function dissolve_short_bg($str_value)
    {
        ...
        if(Diglin_Csstidy_Core::is_important($str_value))
        {
            $important = ' !important';
            $str_value = Diglin_Csstidy_Core::gvw_important($str_value);
        }
