...
class Diglin_Csstidy_Print
{
     ...
     * @version 2.0
     */
    private function _print($plain = false)
    {
     ...
     * @version 1.0
     */
    private function _seeknocomment($key, $move) {
        $go = ($move > 0) ? 1 : -1;
     ...
     * @version 1.0
     */
    private function _convert_raw_css()
    {
        $this->tokens = array();
        ...
        foreach ($this->css as $medium => $val)
        {
            ...
            if ($medium != DEFAULT_AT) {
                $this->parser->add_token(AT_START, $medium, true);
            }

            foreach ($val as $selector => $vali)
            {
                if ($this->parser->get_cfg('sort_properties')) ksort($vali);
                $this->parser->add_token(SEL_START, $selector, true);

                foreach ($vali as $property => $valj)
                {
                    $this->parser->add_token(PROPERTY, $property, true);
                    $this->parser->add_token(VALUE, $valj, true);
                }

                $this->parser->add_token(SEL_END, $selector, true);
            }

            if ($medium != DEFAULT_AT) {
                $this->parser->add_token(AT_END, $medium, true);
            }
        }
    }
     ...
     * @version 1.0
     */
    private function _htmlsp($string, $plain)
    {
        if (!$plain) {
